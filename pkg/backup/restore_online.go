...
func sendAddRemoteSSTs(
	...
	if err := job.NoTxn().UpdateStatus(ctx, "Splitting and distributing spans"); err != nil {
		return 0, 0, err
	}
	...
	if err := job.NoTxn().UpdateStatus(ctx, ""); err != nil {
		return 0, 0, err
	}
...
func (r *restoreResumer) maybeCalculateTotalDownloadSpans(
	...
	if err := r.job.NoTxn().UpdateStatus(ctx, "Calculating total download size..."); err != nil {
	...
	if err := r.job.NoTxn().Update(ctx, func(txn isql.Txn, md jobs.JobMetadata, ju *jobs.JobUpdater) error {
		...
		md.Progress.Status = fmt.Sprintf("Downloading %s of restored data...", sz(total))
