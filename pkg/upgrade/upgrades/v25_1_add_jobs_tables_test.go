...
func TestBackfillJobsTablesAndColumns(t *testing.T) {
	...
	require.NoError(t, getJob(1003).NoTxn().UpdateStatus(ctx, "up that hill"))
	...
	require.NoError(t, getJob(1005).NoTxn().UpdateStatus(ctx, "up that hill"))
	...
	require.NoError(t, getJob(1006).NoTxn().UpdateStatus(ctx, "up that hill"))
	require.NoError(t, getJob(1007).NoTxn().FractionProgressed(ctx, jobs.FractionUpdater(.5)))
	require.NoError(t, getJob(1007).NoTxn().UpdateStatus(ctx, "up that hill"))
	resume[1006] <- nil
