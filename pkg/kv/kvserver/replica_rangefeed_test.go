...
func TestReplicaRangefeedOriginIDFiltering(t *testing.T) {
	...
	aArgs := putArgs(roachpb.Key("b"), []byte("b1"))
	_, err := kv.SendWrapped(ctx, store.TestSender(), aArgs)
	require.Nil(t, err)

	bArgs := putArgs(roachpb.Key("b"), []byte("b2"))
	_, err = kv.SendWrappedWith(ctx, store.TestSender(), kvpb.Header{WriteOptions: &kvpb.WriteOptions{OriginID: 1}}, bArgs)
	require.Nil(t, err)
	...
	if err := store.DB().Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		batch := txn.NewBatch()
		batch.Put(roachpb.Key("c"), []byte("c4"))
		return txn.Run(ctx, batch)
	}); err != nil {
		t.Fatal(err)
	}
	...
	expCValue.InitChecksum([]byte("c")) // kv.Txn sets value checksum
