...
func TestChangefeedRetryableError(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		require.NoError(t, err)
		require.Contains(t, job.Progress().Status, "synthetic retryable error")
		...
		require.Contains(t, status, "synthetic retryable error")
...
func TestChangefeedOrderingWithErrors(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		testutils.SucceedsSoon(t, func() error {
			status, err := feedJob.FetchStatus()
			if err != nil {
				return err
			}
...
func TestChangefeedOnErrorOption(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		t.Run(`pause on error`, func(t *testing.T) {
			...
			job, err := registry.LoadJob(context.Background(), jobID)
			require.NoError(t, err)
			require.Contains(t, job.Progress().Status, "job failed (should fail with custom error) but is being paused because of on_error=pause")
