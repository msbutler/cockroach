...
func moveConflictingSystemTable(
	...
	err := sql.DescsTxn(ctx, p.ExecCfg(), func(ctx context.Context, txn *kv.Txn, descsCol *descs.Collection) error {
		...
		if _, err := p.ExecCfg().InternalExecutor.ExecEx(ctx, "rename-system-table", txn,
			sessiondata.InternalExecutorOverride{User: security.NodeUserName()}, renameStmt); err != nil {
			return err
		}
		return nil
	})
...
func allocateDescriptorRewrites(
	...
	if descriptorCoverage == tree.AllDescriptors || restoreSystemUsers {
		...
		// Assign the next valid ID to the temporary system database that we use
		// during the restore.
...
func doRestorePlan(
	...
	if err := checkIntroducedSpanCorruption(sqlDescs, mainBackupManifests, endTime,
