...
func getReintroducedSpans(
	...
	// in the backup. While investigating #88042, it was discovered that
	// during revision history backups, a table can get included in
	// manifest.DescriptorChanges, causing its spans to get backed up, but _not_
	// in manifest.Descriptors. Therefore, to find all descriptors covered in the
	// backup that were offline at backup time, we must find all tables in
	// manifest.DescriptorChanges whose last change brought the table offline.
