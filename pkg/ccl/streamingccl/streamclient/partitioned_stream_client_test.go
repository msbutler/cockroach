...
`)
	...
	h.SysSQL.QueryRow(t, "crdb_internal.create_sql_schema_telemetry_job()").Scan(&telemetryJobID)
	_, err = client.Plan(ctx, streampb.StreamID(telemetryJobID))
	require.True(t, testutils.IsError(err, fmt.Sprintf("job %d is not a replication stream job", telemetryJobID)), err)
