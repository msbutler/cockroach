	...
	datadriven.Walk(t, datapathutils.TestDataPath(t), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "create-replication-clusters":
				...
				if hey {
					t0 := time.Now()
					retainedCeil := t0.Truncate(time.Microsecond).Add(time.Microsecond)
					t0TZ, _ := tree.MakeDTimestampTZ(retainedCeil, time.Nanosecond)
					t0TZ1, _ := tree.MakeDTimestampTZ(retainedCeil, time.Microsecond)
					t.Fatalf("nano %s, micro %s", t0TZ, t0TZ1)
				}
			...
			case "compare-tenant-fingerprints":
				...
				if d.HasArg("table_fingerprints") {
					if d.HasArg("table_fingerprints") {
						startTime := stringToHLC(t, from).GoTime()
						microSecondRFC3339Format := "2006-01-02 15:04:05.999999"
						startTimeFmt := startTime.Format(microSecondRFC3339Format)
						t.Fatalf("Formatted %s; GoTime %s", startTimeFmt, startTime.String())
					}
					require.NoError(t, replicationutils.InvestigateFingerprints(ctx,
						ds.replicationClusters.SrcTenantConn,
						ds.replicationClusters.DestTenantConn, stringToHLC(t, from), stringToHLC(t, to)))
