...
func (sbs scheduledBackupSpecs) scheduledBackupCmd() string {
	...
	cmd := fmt.Sprintf(`CREATE SCHEDULE %q FOR %s RECURRING '%s' FULL BACKUP '@weekly' WITH SCHEDULE OPTIONS first_run = 'now', %s`,
...
func (bd *backupDriver) monitorBackups(ctx context.Context) {
	...
	for {
		...
		scheduleCountQuery := fmt.Sprintf(`SELECT count(*) FROM [SHOW SCHEDULES] WHERE label=%q AND schedule_status='ACTIVE'`, scheduleLabel)
		...
		if backupCount >= bd.sp.backup.backupsIncluded {
			pauseSchedulesQuery := fmt.Sprintf(`PAUSE SCHEDULES WITH x AS (SHOW SCHEDULES) SELECT id FROM x WHERE label = %q`, scheduleLabel)
