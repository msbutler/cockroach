...
func checkDetachedRestore(
	...
	for {
		...
		case <-succeededJobTick.C:
			...
			if err != nil {
				return errors.Wrapf(err, "failed to open connection to sql server")
			}

			var status string
			if err = conn.QueryRow(`SELECT status FROM [SHOW JOBS] WHERE job_id = $1`, jobID).Scan(
				&status); err != nil {
				return errors.Wrapf(err, "failed to check restore job status")
			}
