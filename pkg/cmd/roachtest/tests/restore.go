...
func checkDetachedRestore(ctx context.Context, t test.Test, c cluster.Cluster,
	...
	for {
		...
		case <-succeededJobTick.C:
			...
			require.NoError(t, err, "failed to open connection to sql server")

			var status string
			err = conn.QueryRow(`SELECT status FROM [SHOW JOBS] WHERE job_id = $1`, jobID).Scan(
				&status)
			require.NoError(t, err, "failed to check restore job status")
