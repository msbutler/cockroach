...
func (sc *SchemaChanger) runBackfill(ctx context.Context) error {
	...
	tableDesc, err := sc.updateJobStatus(ctx, StatusBackfill)
	if err != nil {
		return err
	}
...
func (sc *SchemaChanger) validateConstraints(
	...
	_, err := sc.updateJobStatus(ctx, StatusValidation)
	if err != nil {
		return err
	}
...
func (sc *SchemaChanger) distIndexBackfill(
	...
	if writeAsOf.IsEmpty() {
		status := jobs.Status("scanning target index for in-progress transactions")
		if err := sc.job.NoTxn().UpdateStatus(ctx, status); err != nil {
		...
		if err := sc.job.NoTxn().UpdateStatus(ctx, StatusBackfill); err != nil {
			return errors.Wrapf(err, "failed to update running status of job %d", errors.Safe(sc.job.ID()))
		}
	} else {
...
// updateJobStatus updates the status field in the job entry
...
func (sc *SchemaChanger) updateJobStatus(
	ctx context.Context, status jobs.Status,
	...
	err = DescsTxn(ctx, sc.execCfg, func(ctx context.Context, txn isql.Txn, col *descs.Collection) (err error) {
		...
		if updateJobRunningProgress && !tableDesc.Dropped() {
			if err := sc.job.WithTxn(txn).UpdateStatus(ctx, status); err != nil {
				return errors.Wrapf(err, "failed to update running status of job %d", errors.Safe(sc.job.ID()))
			}
		}
		return nil
...
func (sc *SchemaChanger) runStateMachineAfterTempIndexMerge(ctx context.Context) error {
	...
	return sc.txn(ctx, func(
		...
		if sc.job != nil {
			if err := sc.job.WithTxn(txn).UpdateStatus(ctx, runStatus); err != nil {
