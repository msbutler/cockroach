...
func (desc *Mutable) InitializeImport(importType descpb.TableDescriptor_ImportType) error {
	if desc.ImportTypeInProgress != descpb.TableDescriptor_NO_IMPORT {
		return errors.AssertionFailedf("IMPORT already in progress with import type %v and epoch %v",
			desc.ImportTypeInProgress, desc.ImportEpoch)
	}
	desc.ImportEpoch++
