...
func WaitForNoIngestingNodes(
	...
	for retries.Next() {
		...
		if timeutil.Since(lastStatusUpdate) > statusUpdateFrequency {
			status := jobs.Status(fmt.Sprintf("waiting for all nodes to finish ingesting writing before proceeding: %s", err))
			if statusErr := job.NoTxn().UpdateStatus(ctx, status); statusErr != nil {
